#!/usr/bin/env python3
"""
Interactive multi-model prompt console (vLLM) for fast prompt iteration.

Run on an allocated GPU compute node (interactive PBS session) in the `M4R_llama3x_vllm`
env. The three LLaMA rungs are loaded once and kept resident in GPU memory at the same
time. Each round re-reads the prompt file; its FIRST line selects which resident model
answers, e.g.:

    #model: 3_1_8b
    <your prompt from here on>

Editing only the first line switches models instantly, with no reloading. Use this to
eyeball a prompt (e.g. the methods-only prompt) on all three models before committing to a
full step0/step1 run.

Paths default to this folder (`prompts/linear_probe.txt`, `outputs/`); model weights are
absolute (machine-specific), same as llm_gen/.
"""
...
# The three NM-SCRC rungs (weak / mid / strong). Weights are absolute, machine-specific.
MODEL_PATH_MAP = {
...
# Conservative per-model fraction of TOTAL GPU memory each resident vLLM engine may use
# (weights + KV cache + activations). The sum must stay below 1.0 so all models coexist on
# one GPU (defaults ~0.85 fit a 46GB L40S). Override at runtime with --gpu-frac.
DEFAULT_GPU_FRAC = {
    "3_1_8b": 0.45,
    "3_2_3b": 0.25,
    "3_2_1b": 0.15,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Interactive multi-model prompt console with vLLM")
    parser.add_argument(
        "--models",
    ...
    parser.add_argument(
        "--prompt-file",
        default=str(CONSOLE_ROOT / "prompts" / "linear_probe.txt"),
        help="Path to plain-text prompt file (first line selects the model)",
    )
    parser.add_argument(
        "--output-dir",
        default=str(CONSOLE_ROOT / "outputs"),
