...
"""
Step 1 (upstream): extract hidden states and generate method-answer completions.

Self-contained version of the original `SCRC/1_hidden_state_gen.py`, repointed at the
NMSCRC folder layout. For each methods-only prompt it:
  - runs a forward pass and stores the final-layer / final-token hidden state, and
  - greedily decodes the model's method-selection answer (vLLM temperature=0 equivalent).

Inputs  : NMSCRC/data/prompts/{D_train, mini-StatQA} for methods-only.csv  (from step0)
          + local LLaMA weights under /…/files/models/ (absolute, machine-specific).
Outputs : NMSCRC/data/data-full/{model}_{split}_hs.npy   (hidden states)
          NMSCRC/data/data-full/{model}_{split}_data.csv  (Origin-Answer-compatible)
          where split is `medium` (= D_train block) and `hard` (= mini-StatQA test).

Downstream: `notebooks/pool.ipynb` concatenates medium+hard -> `{model}_pool_*`, which are
the inputs the nmscrc package (check_paths.py) requires.

Run on a GPU node in the `M4R_llama3x` env — see llm_gen/jobs/ for the PBS launchers.
Selectable model via --model_type (3_1_8b / 3_2_1b / 3_2_3b). Outputs are frozen and shipped;
re-running is only needed to regenerate from scratch.
"""
...
# NMSCRC project root (.../StatQA/NMSCRC); all relative paths resolve against it.
NMSCRC_ROOT = Path(__file__).resolve().parents[1]


# Local LLaMA model registry (absolute, machine-specific paths — unchanged from SCRC).
MODEL_TYPE_TO_PATH = {
...
# On-disk output prefixes — EXACT strings the nmscrc package expects (nmscrc.MODELS).
MODEL_TYPE_TO_NAME = {
    "3_1_8b": "llama3_1_8b",
    "3_2_1b": "llama3_2_1b",
    "3_2_3b": "llama3_2_3b",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract hidden states and method completions for NMSCRC Step 1."
    )
    parser.add_argument(
        "--train_csv",
        type=str,
        default="data/prompts/D_train for methods-only.csv",
        help="Training (methods-only) prompt CSV, relative to the NMSCRC root.",
    )
    parser.add_argument(
        "--test_csv",
        type=str,
        default="data/prompts/mini-StatQA for methods-only.csv",
        help="Test (methods-only) prompt CSV, relative to the NMSCRC root.",
    )
    parser.add_argument(
        "--model_type",
        type=str,
    ...
    parser.add_argument(
        "--hidden_state_dir",
        type=str,
        default="data/data-full",
        help="Directory for hidden state (.npy) outputs, relative to the NMSCRC root.",
    )
    parser.add_argument(
        "--origin_answer_dir",
        type=str,
        default="data/data-full",
        help="Directory for Origin Answer-compatible CSV outputs, relative to the NMSCRC root.",
    )
    parser.add_argument(
...
def resolve_repo_path(path_value: str) -> str:
    if os.path.isabs(path_value):
        return path_value
    return os.path.join(NMSCRC_ROOT, path_value)
...
def main() -> None:
    ...
    # Keep relative paths stable regardless of launch directory.
    os.chdir(NMSCRC_ROOT)
